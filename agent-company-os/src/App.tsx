...
function decisionTone(decision: ProductIdea['decision']) {
  return decision === 'Build' ? 'build' : decision === 'Kill' ? 'kill' : 'improve'
}

function ProductDecisionBadge({ decision }: { decision: ProductIdea['decision'] }) {
  return <span className={`decision-badge decision-${decisionTone(decision)}`}>{decision}</span>
}

function ProductIdeaCard({ product, agents }: { product: ProductIdea; agents: Agent[] }) {
  const agentNames = product.assignedAgents.map((id) => agents.find((agent) => agent.id === id)?.name).filter(Boolean).join(', ')
  return <article className={`product-card product-decision-${decisionTone(product.decision)}`}>
    <div className="product-card-head"><div><span className="product-stage-chip">{product.stage}</span><b>{product.productName}</b></div><ProductDecisionBadge decision={product.decision} /></div>
    <div className="confidence-row"><span>Confidence</span><strong>{product.confidenceScore}%</strong><i style={{ width: `${product.confidenceScore}%` }} /></div>
    <div className="product-scan-grid"><div><span>Target niche</span><p>{product.targetNiche}</p></div><div><span>Pain</span><p>{product.customerPain}</p></div></div>
    <div className="next-action-box"><span>Next action</span><b>{product.nextAction}</b></div>
    <details className="product-details"><summary>Secondary details</summary><div className="product-meta"><span>{product.productType}</span><span>{product.expectedPrice}</span><span>{product.createdAt}</span></div><p>{product.productPromise}</p><p><b>Validation:</b> {product.validationNotes}</p><small>Assigned: {agentNames}</small></details>
  </article>
}

function ProductLab({ products, agents }: { products: ProductIdea[]; agents: Agent[] }) {
  return <section className="product-lab-page"><div className="page-head"><div><p className="eyebrow">Product Lab</p><h1>Premium B2B Product Pipeline</h1><span>Decide what to build, improve or kill — one product candidate at a time.</span></div></div><div className="decision-strip glass-panel"><div><b>Operating rule</b><span>Every card must answer: who is it for, what pain does it solve, what is the next action, and what is the decision?</span></div></div><div className="product-pipeline">{productStages.map((stage) => { const stageProducts = products.filter((product) => product.stage === stage); return <div className="product-stage glass-panel" key={stage}><div className="stage-head"><h3>{stage}</h3><span>{stageProducts.length}</span></div>{stageProducts.map((product) => <ProductIdeaCard key={product.id} product={product} agents={agents} />)}</div> })}</div></section>
}

function DemandTests({ products }: { products: ProductIdea[] }) {
  return <section className="lab-table-page"><div className="page-head"><div><p className="eyebrow">Demand Tests</p><h1>48h Validation Metrics</h1><span>Read signal quality fast: traffic, intent, signups and buy-now clicks.</span></div></div><div className="demand-grid">{products.map((product) => <article className={`demand-card glass-panel product-decision-${decisionTone(product.decision)}`} key={product.id}><div className="product-card-head"><div><span className="product-stage-chip">{product.landingPageStatus}</span><b>{product.productName}</b></div><ProductDecisionBadge decision={product.decision} /></div><div className="demand-metrics"><div><span>CTR</span><b>{product.ctr}%</b></div><div><span>CPC</span><b>£{product.cpc}</b></div><div><span>Signups</span><b>{product.emailSignups}</b></div><div><span>Buy-now</span><b>{product.buyNowClicks}</b></div><div><span>Conv.</span><b>{product.conversionRate}%</b></div><div><span>Budget</span><b>£{product.adBudget}</b></div></div><details className="product-details"><summary>Traffic details</summary><p>{product.landingPageUrl || 'No landing page URL yet'}</p><small>{product.impressions} impressions · {product.clicks} clicks</small></details></article>)}</div></section>
}

function ContentEngine({ products }: { products: ProductIdea[] }) {
  return <section className="knowledge-layout"><div className="glass-panel kb-form"><p className="eyebrow">Content Engine</p><h2>Demand-Test Content</h2><p className="mission">Use content to test pain, promise and market pull before overbuilding.</p></div><div className="knowledge-grid content-grid">{products.map((product) => <article className="knowledge-card glass-panel content-card" key={product.id}><p className="eyebrow">{product.platform} · {product.contentStatus}</p><h3>{product.productName}</h3><div className="next-action-box"><span>Primary CTA</span><b>{product.CTA}</b></div><p><b>Main angle:</b> {product.contentAngles[0]}</p><p><b>Best hook:</b> {product.hooks[0]}</p><details className="product-details"><summary>More content assets</summary><p><b>Angles:</b> {product.contentAngles.join(' / ')}</p><p><b>Short videos:</b> {product.shortVideoIdeas.join(' / ')}</p></details></article>)}</div></section>
}

function EmailFunnel({ products }: { products: ProductIdea[] }) {
  return <section className="knowledge-layout"><div className="glass-panel kb-form"><p className="eyebrow">Email Funnel</p><h2>B2B Sales Engine Assets</h2><p className="mission">Turn validation traffic into leads, intent and simple premium offers.</p></div><div className="knowledge-grid funnel-grid">{products.map((product) => <article className="knowledge-card glass-panel funnel-card" key={product.id}><p className="eyebrow">{product.funnelStatus}</p><h3>{product.productName}</h3><div className="next-action-box"><span>Lead magnet</span><b>{product.leadMagnet}</b></div><p><b>Offer email:</b> {product.offerEmail}</p><small>Bonus: {product.bonus} · Deadline: {product.deadline}</small><details className="product-details"><summary>Email sequence</summary><p>{product.emailSequence.join(' → ')}</p></details></article>)}</div></section>
}
